...
import { Play, Pause, RotateCcw, Zap, User, Ban } from 'lucide-react';
...
export const LiveVisualization: React.FC<Props> = ({ results, config }) => {
  ...
  // --- LOGIQUE DE POSITIONNEMENT (COORDONNÉES ABSOLUES %) ---
  const getAgentPosition = (
    ...
    // --- NOUVELLES CONSTANTES DE LAYOUT ---
    ...
    switch (status) {
      ...
      case 'DROPPED_EXEC':
      case 'DROPPED_RESULT':
        // Colonne de rejet (Trash bin) à gauche
        // Empilement de bas en haut, unifié pour tous les rejets
        const colsDropped = 4;
        const colDropped = globalDroppedIndex % colsDropped;
        const rowDropped = Math.floor(globalDroppedIndex / colsDropped);
        
        // Ajustement pour centrer dans le conteneur de 12% de large
        // Start à +2% pour éviter le bord gauche
        x = COL_DROPPED_START + 2 + (colDropped * 2.5);
        // Start from bottom 75% to ensure it's above the bottom edge (approx 80%)
        y = 75 - (rowDropped * 2); 
        
        if (y < 20) opacity = 0; // Hide if overflowing top
        break;

      case 'QUEUED_EXEC':
        // Remplissage de bas en haut dans la colonne 1
        const colsQ1 = 5; 
        const colQ1 = indexInGroup % colsQ1;
        const rowQ1 = Math.floor(indexInGroup / colsQ1);
        
        x = COL_Q1_START + 1.5 + (colQ1 * 3);
        // Start from 75% (bottom is 80%)
        y = 75 - (rowQ1 * 2.5); 
        
        // Hide if overflowing top of container (approx 15%)
        if (y < 16) { opacity = 0; } 
        break;

      case 'PROCESSING':
        // Utilise l'ID de serveur réellement assigné par le moteur
        // Fallback sur le modulo si l'ancienne logique tourne encore (legacy safety)
        const serverId = agent.assignedServerId !== undefined ? agent.assignedServerId : agent.id % config.executionServerCount;
        
        // Zone serveurs entre Y=15% et Y=80%
        const serverAreaHeight = 65;
        const singleServerHeight = serverAreaHeight / config.executionServerCount;
        const slotTopY = 15 + (serverId * singleServerHeight);
  ...
  activeAgents.forEach(agent => {
    ...
    if (agentsByStatus[status]) {
      agentsByStatus[status].push(agent);
    } else {
         agentsByStatus[status] = [agent];
    }
  });
  
  // Calculate specific index for dropped items to stack them together
  const allDropped = [
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100 font-sans overflow-hidden">
      ...
      {/* Barre de Contrôle */}
      <div className="bg-white p-3 border-b border-slate-200 flex items-center justify-between shrink-0 z-20 shadow-sm h-16">
        ...
        <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-slate-500 w-16 text-right">
              {Math.round(1000 / playbackSpeed)} t/s
            </span>
        </div>
      </div>
      ...
      <div className="flex-1 relative bg-slate-50 overflow-hidden select-none">
        ...
        <div className="absolute inset-0 pointer-events-none">
           ...
           {/* TITRES COLONNES */}
           <div className="absolute top-4 w-full flex text-center text-xs font-bold text-slate-400 uppercase tracking-wider">
           ...
           <div className="absolute top-[15%] bottom-[20%] left-[5%] w-[12%] border-2 border-dashed border-red-200 bg-red-50/30 rounded-lg flex flex-col justify-end">
              <div className="absolute bottom-2 w-full text-center text-xs font-bold text-red-300">
                {totalDroppedSoFar} Perdus
              </div>
           </div>

           {/* COLONNE 1 : CONTAINER FILE EXECUTION */}
           {/* Remonté à bottom-20% pour ne pas dépasser */}
           <div className="absolute top-[15%] bottom-[20%] left-[22%] w-[15%] border-2 border-slate-300 bg-white rounded-t-lg border-b-0 flex flex-col justify-end">
              <div className="w-full h-2 bg-slate-200/50 absolute bottom-0"></div>
              {/* Indicateur de capacité */}
              <div className="absolute -right-12 bottom-0 text-xs font-mono text-slate-400 rotate-0">
           ...
           {config.scenario === ScenarioType.CHANNELS_DAMS && (
              <div className={`absolute top-[12%] left-[22%] w-[15%] h-1 transition-colors duration-300 z-20 ${damOpen ? 'bg-green-400 shadow-[0_0_10px_rgba(74,222,128,0.5)]' : 'bg-red-500 shadow-[0_0_15px_rgba(248,113,113,0.8)]'}`}>
           ...
           <div className="absolute top-[15%] bottom-[20%] left-[45%] w-[10%] flex flex-col gap-0">
               {Array.from({length: serverCount}).map((_, i) => (
                 <div key={i} className="w-full flex-1 border-2 border-slate-300 bg-white mb-1 rounded flex items-center justify-center relative shadow-sm">
           ...
           <div className="absolute top-[15%] bottom-[20%] left-[65%] w-[15%] border-2 border-slate-300 bg-white rounded-t-lg border-b-0">
               <div className="absolute -right-12 bottom-0 text-xs font-mono text-slate-400 rotate-0">
           ...
           <div className="absolute top-[40%] bottom-[40%] left-[90%] w-[5%] border-2 border-emerald-200 bg-emerald-50/30 rounded flex items-center justify-center">
        ...
        <div className="absolute inset-0">
          {activeAgents.map(agent => {
             ...
             // Pour les rejets, on calcule l'index global pour un empilement unifié
             ...
             return (
               <div
                 ...
                 style={{
                   ...
                   top: `${y}%`,
                   opacity: opacity,
                   transform: `translate(-50%, -50%) scale(${scale})`, // Centrage précis
        ...
        {/* LÉGENDE STATIQUE */}
        <div className="absolute bottom-4 right-4 bg-white/90 p-3 rounded-lg border border-slate-200 shadow-sm text-xs text-slate-600 backdrop-blur-sm">
