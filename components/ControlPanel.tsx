...
// Composant interne pour l'étiquette avec Tooltip
...
export const ControlPanel: React.FC<Props> = ({ config, setConfig, onRun, isRunning }) => {
  ...
  return (
    <div className="w-full md:w-80 bg-slate-50 border-r border-slate-200 p-4 overflow-y-auto h-full flex flex-col gap-6">
      ...
      <div className="space-y-4 border-t border-slate-200 pt-4">
        ...
        <div>
          <LabelWithTooltip 
            ...
            help="Nombre de processeurs disponibles pour exécuter les tests. Plus K est élevé, plus la file d'attente se vide rapidement."
          />
        ...
        <div className="flex items-center justify-between">
           <LabelWithTooltip 
             label="Backup Activé"
             help="Active un système de secours qui sauvegarde les résultats si la file de retour est pleine, évitant ainsi la perte de données (0% de pages blanches)."
           />
           <button 
      ...
      <div className="space-y-4 border-t border-slate-200 pt-4">
        ...
        <div className="p-3 bg-white rounded border border-slate-200 shadow-sm">
          ...
          <div className="mb-3">
            <LabelWithTooltip 
              ...
              help="Probabilité qu'un étudiant soumette un travail à chaque tick. 100% = 1 soumission/tick."
            />
            <div className="flex items-center gap-2">
                <input 
          ...
          <div>
            <LabelWithTooltip 
              label="Durée Service (µ)" 
              help="Temps (en ticks) nécessaire au serveur pour traiter une soumission de ce type."
            />
            <div className="flex items-center gap-2">
                <input 
        ...
        {/* PREPA Population (Only for Channels) */}
        {config.scenario === ScenarioType.CHANNELS_DAMS && (
          <div className="p-3 bg-white rounded border border-slate-200 shadow-sm">
            ...
            <div className="mb-3">
              <LabelWithTooltip 
                label="Taux d'arrivée (λ)" 
                help="Probabilité d'arrivée pour la population PREPA (souvent plus faible)."
              />
              <div className="flex items-center gap-2">
                <input 
                  type="range" min="0.01" max="0.5" step="0.01" 
                  value={config.arrivalRatePrepa} 
                  onChange={(e) => handleChange('arrivalRatePrepa', parseFloat(e.target.value))}
                  className="flex-1 h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-purple-600"
                />
                <span className="text-xs font-mono w-12 text-center bg-slate-50 px-1 rounded border border-slate-200 text-slate-600">
                    {Math.round(config.arrivalRatePrepa * 100)}%
                </span>
              </div>
            </div>
            ...
            <div>
              <LabelWithTooltip 
                label="Durée Service (µ)" 
                help="Temps de traitement pour la population PREPA (souvent plus long)."
              />
              <div className="flex items-center gap-2">
                <input 
                  type="range" min="10" max="100" step="5" 
                  value={config.serviceDurationPrepa} 
                  onChange={(e) => handleChange('serviceDurationPrepa', parseInt(e.target.value))}
                  className="flex-1 h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-purple-600"
                />
                <span className="text-xs font-mono w-12 text-center bg-slate-50 px-1 rounded border border-slate-200 text-slate-600">
                    {config.serviceDurationPrepa}t
                </span>
              </div>
            </div>
          </div>
      ...
      {config.scenario === ScenarioType.CHANNELS_DAMS && (
        <div className="space-y-4 border-t border-slate-200 pt-4">
          ...
          <div>
            <LabelWithTooltip 
              label="Durée Blocage (tb)" 
              help="Durée de fermeture du barrage pour les ING. Permet de réguler le flux pour laisser passer les PREPA."
            />
            <div className="flex items-center gap-2">
              <input 
                type="range" min="0" max="100" step="10" 
                value={config.damBlockDuration} 
                onChange={(e) => handleChange('damBlockDuration', parseInt(e.target.value))}
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Bloqué pour {config.damBlockDuration}, ouvert pour {config.damBlockDuration / 2}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="mt-auto pt-4">
         <button 
           onClick={onRun}
           disabled={isRunning}
           className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
         >
